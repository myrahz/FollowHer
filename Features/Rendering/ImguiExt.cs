...
/// <summary>Small self-contained ImGui helpers for the rule editor's drag-reorder list and delete
/// confirmation - ported from ReAgent's own ImguiExt.cs (same pattern, same ImGui.NET version).</summary>
public static class ImguiExt
{
    ...
    public static bool EnumerableComboBox<T>(string displayText, IEnumerable<T> items, ref T current)
    {
        ...
        if (ImGui.BeginCombo(displayText, $"{current}"))
        {
            ...
            {
                var selected = Equals(item, current);
